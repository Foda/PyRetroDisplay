...
class Weather(displayio.Group):
    ...
    def update_weather(self, the_weather):
        ...
        self.temp_text.text = str(int(weather_main.get('temp')))
    ...
    def update_time(self):
        ...
        if current.tm_hour > 12:
            self.time_text.text = "{:d}:{:02d}".format(current.tm_hour - 12, current.tm_min)
        else:
            self.time_text.text = "{:d}:{:02d}".format(current.tm_hour, current.tm_min)

        self.am_pm.x = self.time_text.width + 4
        self.am_pm.text = 'PM' if current.tm_hour > 12 else 'AM'
